...
int main(int argc, char* argv[])
{
	...
	MPI_Isend(&f[i1], 1, MPI_DOUBLE, prev, 0, MPI_COMM_WORLD, &requests[0]);
	MPI_Isend(&f[i2], 1, MPI_DOUBLE, next, 0, MPI_COMM_WORLD, &requests[1]);

	MPI_Irecv(&f[i0], 1, MPI_DOUBLE, prev, 0, MPI_COMM_WORLD, &requests[2]);
	MPI_Irecv(&f[i3], 1, MPI_DOUBLE, next, 0, MPI_COMM_WORLD, &requests[3]);
